...
resource "ibm_container_vpc_cluster" "adm_cluster" {
    ...
    wait_till         = "MasterNodeReady"
    disable_public_service_endpoint = false
    resource_group_id = data.ibm_resource_group.adm_resource_group.id
    tags              = ["env:${var.environment}","vpc:${var.vpc_name}","schematics:${var.schematics_workspace_name}"]
    zones {
        subnet_id = module.adm_subnets.subnet1_id
        name      = "${var.region}-1"
    }
    zones {
        subnet_id = module.adm_subnets.subnet2_id
        name      = "${var.region}-2"
    }
    zones {
        subnet_id = module.adm_subnets.subnet3_id
        name      = "${var.region}-3"
    }
}
*/
